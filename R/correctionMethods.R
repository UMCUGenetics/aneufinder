...
#' GC correction and sequenceability correction method.
#'
#' Two methods are available for GC correction:  Option \code{method='quadratic'} uses the method described in the Supplementary of \code{citation("AneuFinder")}. Option \code{method='loess'} uses a loess fit to adjust the read count.
#'
#' @param binned.data.list A \code{list} with \code{\link{binned.data}} objects or a list of filenames containing such objects.
...
#'
#' @export
correctGCSC <- function(binned.data.list, sequenceability.bins.list, same.binsize=FALSE, method='loess', return.plot=FALSE, bins=NULL) {
  ...
  ### Loop over all bin entries ###
    ...
    blist.gc.df <- as.data.frame(blist.gc)
    sequenceability.df <- as.data.frame(sequenceability)

    blist.final.df <- merge (blist.gc.df, sequenceability.df, by=c("seqnames", "start", "end"))
    ...
    stopTimedMessage(ptm)
		...
		if (!return.plot) {
  		## Spikyness
  		attr(blist.final, 'qualityInfo')$spikiness <- qc.spikiness(blist.final$counts)
  		## Shannon entropy
  		attr(blist.final, 'qualityInfo')$entropy <- qc.entropy(blist.final$counts)
  		## ID
  		attr(blist.final, 'ID') <- attr(binned.data, 'ID')
  		# Return
  		binned.data.list[[i1]] <- blist.final
		} else {
		  binned.data.list[[i1]] <- plots
		}
